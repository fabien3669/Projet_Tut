...
public class Model {
    ...
    public void joueIA() {
        ...
        if (bateauTouche){
            ...
            if (getBateauAt(last).getCaseTouché()!=1){
    ...
    private void coupAutour(Coordonnee last) {
        ...
        Coordonnee coordonnee = new Coordonnee(last.getLigne()+dl, last.getColonne()+dc);
        ...
        if (isBateauAt(coordonnee)){
            if (getBateauAt(coordonnee).getState().equals(Etat.TOUCHE.getState())){
                directionConnue=true;
            }
            Bateau b = getBateauAt(coordonnee);
            updateBateau(b);
            if (getBateauAt(coordonnee).getState().equals(Etat.COULE.getState())){
                bateauTouche=false;
                directionConnue=false;
            }
        }
