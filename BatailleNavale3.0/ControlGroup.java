...
public class ControlGroup {
    ...
    public ControlGroup(Model model) {
        ...
        this.controlForm = new ControlForm(frame, model, fenetre, controlButtonJeu);
        ...
        if (but.equals(fenetre.getJbMenu())){
            ...
            if (model.getModeDeJeu()==model.JOUEUR_CONTRE_JOUEUR){
                ...
                if (model.getIndiceDernierCoupJoueur1()!=0){
                    ...
                    if (model.partieTerminee()){
                        frame.setFinPartie(frame.DEFAITE);
                        fenetre.unsetControlButtonGame(this);
                    }
                    model.changeJoueurEnCours();
                }
                fenetre.affichageTirs();
            ...
            if (!model.isaJoue()){
                ...
                if (model.isCoupValide(xy)){
                    ...
                    if (model.partieTerminee()){
                        fenetre.unsetControlButtonGame(this);
                        frame.setFinPartie(frame.VICTOIRE);
                    }
                    else{
                        model.setaJoue(true);
                ...
                frame.setInformation("<html>&emsp;&emsp;&emsp;Vous avez déjà joué,<br> veuillez terminer votre tour.</html>", frame.AFFICHAGE_LABEL);
            }
        }
    }
}
...
class ControlForm extends ControlButton implements ActionListener{
    ...
    public ControlForm(MyJFrame frame, Model model, Fenetre fenetre, ControlButtonJeu controlButtonJeu) {
        super(model, fenetre);
        this.frame = frame;
        this.controlButtonJeu = controlButtonJeu;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (but.equals(frame.getNouvellePartie())) {
            fenetre.unsetControlButtonGame(controlButtonJeu);
            fenetre.setControlButtonGame(controlButtonJeu);
            model.initModel();
            frame.setFormulaireModeJeu();
        }
